    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        ...
        <div>
            <span class="badge bg-primary p-2">
                <i class="fas fa-calendar-alt me-1"></i> Ano Letivo: {{ $anoLectivo ?? date('Y') }}
            </span>
        </div>
    </div>
        ...
        <div class="card-body py-3">
            <div class="d-flex justify-content-center gap-2">
                <div class="btn-group" role="group">
                    <a href="{{ route('aluno.notas', ['trimestre' => 1]) }}" 
                       class="btn btn-sm {{ ($trimestre ?? 1) == 1 ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fas fa-calendar-week me-1"></i> 1º Trimestre
                    </a>
                    <a href="{{ route('aluno.notas', ['trimestre' => 2]) }}" 
                       class="btn btn-sm {{ ($trimestre ?? 1) == 2 ? 'btn-primary' : 'btn-outline-primary' }}">
                        <i class="fas fa-calendar-week me-1"></i> 2º Trimestre
                    </a>
                    <a href="{{ route('aluno.notas', ['trimestre' => 3]) }}" 
                       class="btn btn-sm {{ ($trimestre ?? 1) == 3 ? 'btn-primary' : 'btn-outline-primary' }}">
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <span class="badge bg-secondary">
                        ...
                        Turma: {{ $turma->nome ?? 'A aguardar atribuição' }}
                    </span>
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr class="{{ !$temNota ? 'table-light' : '' }}">
                            ...
                            <td class="text-center">
                                @if($temNota)
                                    <span class="fw-bold">{{ number_format($mac ?? 0, 1) }}</span>
                                @else
                            ...
                            <td class="text-center">
                                @if($temNota)
                                    <span class="fw-bold">{{ number_format($prova ?? 0, 1) }}</span>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            </td>
                            <td class="text-center">
                                @if($temNota && $mediaTrimestral !== null)
                                    <span class="badge bg-light text-dark">
                                        {{ number_format($mediaTrimestral, 1) }}
                                    </span>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            ...
                            <td class="text-center">
                                @if($temNota)
                                    <strong class="text-{{ $mediaFinalValue >= 10 ? 'success' : 'danger' }}">
                                        {{ number_format($mediaFinalValue, 1) }}
                                    </strong>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            ...
                            <td class="text-center">
                                @if($temNota)
                                    <div class="d-flex flex-column align-items-center">
                                        <span class="fw-bold">{{ $faltas }}</span>
                                        <div class="progress w-100" style="height: 4px;">
                                            <div class="progress-bar bg-{{ $percentFaltas > 50 ? 'danger' : ($percentFaltas > 25 ? 'warning' : 'success') }}" 
                                                 style="width: {{ $percentFaltas }}%">
                                            </div>
                                        </div>
                                        <small class="text-muted">{{ $percentFaltas }}%</small>
                                    </div>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            ...
                            <td class="text-center">
                                @if($temNota)
                                    <span class="badge bg-{{ $situacaoClass }} px-3 py-2">
                ...
                <div class="card-body py-3">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 text-end">
                            <h3 class="mb-0 fw-bold text-success">{{ $notas->where('situacao', 'aprovado')->count() + $notas->where('situacao', 'aprovado_exame')->count() }}</h3>
            ...
            <div class="card summary-card border-top-warning">
                <div class="card-body py-3">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 text-end">
                            <h3 class="mb-0 fw-bold text-warning">{{ $notas->where('situacao', 'exame')->count() }}</h3>
            ...
            <div class="card summary-card border-top-danger">
                <div class="card-body py-3">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 text-end">
                            <h3 class="mb-0 fw-bold text-danger">{{ $notas->where('situacao', 'reprovado')->count() }}</h3>
            ...
            <div class="card summary-card border-top-primary">
                <div class="card-body py-3">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 text-end">
                            <h3 class="mb-0 fw-bold text-primary">{{ number_format($notas->avg('media_final') ?? $notas->avg('media_trimestral') ?? 0, 1) }}</h3>
